...
<template>
  <NuxtLayout name="simple">
    <template #toast>
      <ToastSimple :show="toast.show" :text="toast.text" :time="toast.time" @close="toast.show = false" />
    </template>
    <!-- <button @click="showToast('hola')">Hola</button> -->
    <section class="pt-32 flex flex-wrap justify-between w-full">
      <div class="flex flex-wrap justify-start items-center gap-2 w-full h-full md:flex-nowrap">
        <SectionsIndexButtons />
        <ButtonsIcon class="basis-full md:basis-1/12 w-icon-3 my-8 md:my-0 md:py-1" icon="lucide:refresh-ccw"
          @click="resetColors()" />
      </div>
      <section class="mt-8 flex flex-wrap justify-between items-center gap-4">
        ...
        <section class="w-full h-fit xl:h-full flex justify-center items-center grow basis-full xl:basis-3/6">
          <PreviewTheme />
        </section>
      </section>
